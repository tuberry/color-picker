...
export const me = () => ExtensionPreferences.lookupByURL(import.meta.url);
export const trait = o => Util.omap(o, ([k, [x, ...ys]]) => [[k, GObject.ParamSpec[x](k, k, k, GObject.ParamFlags.READWRITE, ...ys)]]);
export const val = (...xs) => ({Properties: trait({value: xs})});
 ...
 * @return {T}
 */
export const tie = (o, s, p) => Util.omap(o, ([k, v]) => [[k, (s.bind(Field[k], v, p?.[k] ?? 'value', Gio.SettingsBindFlags.DEFAULT), v)]]);
export const addActRows = (xs, w) => xs.forEach(ys => w.add(new ActRow(...ys)));

export class Broker {
    static #hook = new WeakMap();
    static #bind = new WeakMap();
    ...
    static tie(source, sourceProp, target, targetProp, to = null, from = null, flag = GObject.BindingFlags.SYNC_CREATE) {
        this.#get(source, target, this.#bind).push(source.bind_property_full(sourceProp, target, targetProp, flag, to, from));
    }

    static untie(source, target) {
        this.#get(source, target, this.#bind).splice(0).forEach(x => x.unbind());
    }

    static attach(tracker, ...args) {
        args.forEach((x, i, a) => i % 2 || this.#get(tracker, x, this.#hook).push(...Object.entries(a[i + 1]).map(ys => x.connect(...ys))));
    }

    static detach(tracker, ...args) {
        args.forEach(x => this.#get(tracker, x, this.#hook).splice(0).forEach(y => x.disconnect(y)));
    }
}
...
export class FoldRow extends Adw.ExpanderRow {
    static {
        GObject.registerClass(val('boolean', false), this);
    }
...
export class Drop extends Gtk.DropDown {
    static {
        GObject.registerClass(val('uint', 0, Gtk.INVALID_LIST_POSITION, 0), this);
    }
...
export class Font extends Gtk.FontDialogButton {
    static {
        GObject.registerClass(val('string', ''), this);
    }
...
export class Help extends Gtk.MenuButton {
    ...
    constructor(label, param, param1) {
        super({iconName: 'help-about-symbolic', hasFrame: false, valign: Gtk.Align.CENTER, ...param1});
        this.set_popover(Util.hook({'notify::visible': w => w.child.select_region(-1, -1)}, // HACK: workaround to grab focus without selecting
            new Gtk.Popover({child: new Gtk.Label({label, selectable: true, useMarkup: true, ...param})})));
    }

    setup(label, error) {
        this.get_popover().child.set_label(label);
...
export class DialogBase extends Adw.Dialog {
    ...
    constructor(title, opt, param) {
        ...
        this.add_controller(Util.hook({'key-pressed': (...xs) => this.$onKeyPress(...xs)}, new Gtk.EventControllerKey()));
        this.$buildContent(opt);
    }

    $buildContent(opt) {
        let {content, filter, title} = this.$buildWidgets(opt),
            close = Util.hook({clicked: () => this.close()}, Gtk.Button.new_with_mnemonic(_GTK('_Cancel'))),
            select = Util.hook({clicked: () => this.$onSelect()}, Gtk.Button.new_with_mnemonic(_GTK('_OK'))),
        ...
        if(filter) {
            ...
            let entry = Util.hook({'search-changed': x => filter.set_search(x.get_text())}, new Gtk.SearchEntry({halign: Gtk.Align.CENTER}));
...
export class AppDialog extends DialogBase {
    ...
    $buildWidgets(opt) {
        let factory = Util.hook({
            ...
            content = Util.hook({activate: () => this.$onSelect()}, new Gtk.ListView({model: select, factory, vexpand: true}));
...
class IconDialog extends DialogBase {
    ...
    $buildWidgets(opt) {
        let factory = Util.hook({
            ...
            title = new Drop([_('All'), _('Normal'), _('Symbolic')]), // TODO: ? use https://gnome.pages.gitlab.gnome.org/libadwaita/doc/main/class.ToggleGroup.html
            model = Gtk.StringList.new(Gtk.IconTheme.get_for_display(Gdk.Display.get_default()).get_icon_names()),
            select = new Gtk.SingleSelection({model: new Gtk.FilterListModel({model, filter})}),
            content = Util.hook({activate: () => this.$onSelect()}, new Gtk.GridView({model: select, factory, vexpand: true}));
...
export class DialogButtonBase extends Box {
    ...
    constructor(opt, param, child, gtype, reset) {
        super();
        this.$opt = opt;
        this.$btn = Util.hook({clicked: () => this.$onClick().then(x => { this.value = x; }).catch(Util.noop)}, new Gtk.Button({child, ...param}));
    ...
    $buildReset() {
        this.append(Util.hook({clicked: () => { this.value = ''; }},
            new Gtk.Button({iconName: 'edit-clear-symbolic', tooltipText: _('Clear')})));
    }

    $buildDND(gtype) {
        this.$btn.add_controller(Util.hook({drop: (...xs) => this.$onDrop(...xs)}, Gtk.DropTarget.new(gtype, Gdk.DragAction.COPY)));
        this.$btn.add_controller(Util.hook({prepare: (...xs) => this.$onDrag(...xs)}, new Gtk.DragSource({actions: Gdk.DragAction.COPY})));
    }
    ...
    set value(value) {
        if(Util.str(value) ? this.$value === value : this.$checkGvalue(value)) return;
        this.$setValue(value);
        this.notify('value');
    }
...
export class App extends DialogButtonBase {
    ...
    $setValue(value) {
        let str = Util.str(value);
        this.$gvalue = str ? Gio.DesktopAppInfo.new(value) : value;
        this.$value = str ? value : value.get_id();
        if(this.$gvalue) this.$btn.child.setup(this.$gvalue.get_icon(), this.$gvalue.get_display_name());
        else this.$btn.child.setup();
    }
...
export class File extends DialogButtonBase {
    ...
    constructor(opt, param, icon = 'document-open-symbolic') {
        super(opt, param, new IconLabel(icon), Gio.File.$gtype, true);
    ...
    $onDrop(_t, value) {
        if(!this.$filter) {
            ...
            Util.fquery(value, Gio.FILE_ATTRIBUTE_STANDARD_CONTENT_TYPE).then(y => {
    ...
    $setValue(value) {
        let str = Util.str(value);
        this.$gvalue = str ? Util.fopen(value) : value;
        this.$value = str ? value : value.get_path() ?? '';
        Util.fquery(this.$gvalue, Gio.FILE_ATTRIBUTE_STANDARD_DISPLAY_NAME, Gio.FILE_ATTRIBUTE_STANDARD_ICON)
            .then(x => this.setup(this.$value, x.get_icon(), x.get_display_name())).catch(() => this.setup(value));
    }
...
export class Icon extends DialogButtonBase {
    ...
    $setValue(value) {
        let str = Util.str(value);
        this.$gvalue = str ? Gio.ThemedIcon.new(value) : value;
        this.$value = str ? value : value.to_string();
        if(this.$gvalue) this.$btn.child.setup(this.$value, this.$value.replace(/-symbolic$/, ''));
        else this.$btn.child.setup();
    }
...
export class PrefsPage extends Adw.PreferencesPage {
    static {
        GObject.registerClass(this);
    }
    ...
    constructor(param, prefs = new Adw.PreferencesGroup()) {
        ...
        this.addActRows = rows => addActRows(rows, prefs);
        this.add(this.$prefs = prefs);
    }
}

export class ActRow extends Adw.ActionRow {
    static {
        GObject.registerClass(this);
    }
    ...
    constructor(...args) {
        ...
        if(prefix instanceof Gtk.CheckButton && prefix.sensitive) {
            ...
            this.set_activatable_widget(suffix.find(x => !(x instanceof Help)) ?? null);
        }
    }
}
...
export class Entry extends Gtk.Stack {
    ...
    $buildWidgets(placeholderText = '', mimeTypes, tooltipText = '') {
        let label = new Gtk.Entry({hexpand: true, sensitive: false, placeholderText}),
            ...
            entry = mimeTypes ? Util.hook({
                activate: w => apply(w),
                'icon-press': w => new Gtk.FileDialog({modal: true, defaultFilter: new Gtk.FileFilter({mimeTypes})})
                                .open(this.get_root(), null).then(x => w.set_text(x.get_path())).catch(Util.noop),
            }, new Gtk.Entry({hexpand: true, enableUndo: true, secondaryIconName: 'document-open-symbolic', placeholderText}))
                : Util.hook({activate: w => apply(w)}, new Gtk.Entry({hexpand: true, enableUndo: true, placeholderText})),
            edit = Util.hook({clicked: () => { entry.set_text(label.text); entry.grab_focus(); this.set_visible_child(entry.parent); }},
                new Gtk.Button({iconName: 'document-edit-symbolic', tooltipText})),
            done = Util.hook({clicked: () => apply(entry)}, new Gtk.Button({

...
import Soup from 'gi://Soup/?v=3.0';
...
export const fname = (...xs) => GLib.build_filenamev(xs);
export const amap = (o, f) => omap(o, ([k, v]) => [[k, f(v)]]);
export const lot = x => x[Math.floor(Math.random() * x.length)];
export const fopen = (...xs) => Gio.File.new_for_path(fname(...xs));
...
export const scap = s => [...s].map((x, i) => i ? x.toLowerCase() : x.toUpperCase()).join('');
